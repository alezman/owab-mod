...
function eps() {
  ...
  if(hasUpgrade(1)) eps = eps.add(300)
  ...
  if(hasEscUpgrade(1)) eps = eps.mul(25)
  ...
  if(hasEpsUpgrade(9)) eps = eps.max(0).pow(1.33)
...
function cpGain() {
  let cpGain = player.energy.max(1).log(1.06).div(8).mul(BUYABLES[3].effect())
...
function totalCPGain() {
  let cpGain = player.totalEnergy.max(1).log(1.06).div(8).mul(BUYABLES[3].effect())
  if(hasEscUpgrade(11)) cpGain = cpGain.mul(ESC_UPGRADES[11].effect())
  if(player.buyables[10] >= 1) cpGain = cpGain.mul(seMult(2))
  if(hasMilestone(1)) cpGain = cpGain.mul(10)
  if(hasEpsUpgrade(7)) cpGain = cpGain.mul(EPS_UPGRADES[7].effect())
...
player.challengeTimes[4] = 0
  ...
  if(isNaN(player.energy)) {
    exportSave()
    alert("Haha L you NaNed thanks for playing my mod of Out with a BANG 😫😫 and start again k so bye -alez")
    hardReset()
  }
}
...
function updateHTML(maxTime){
  ...
  if(player.countdownPoints.gt(0) && !player.loreChangeDetect&&!player.gameBegun){
    if(document.getElementById("lore2"))document.getElementById("lore2").innerHTML = "You died L bozo cringe hope you ragequit because ain't there no timewalls in this game 😘😘"
  }else if (player.timesEscaped > 0){
    if(document.getElementById("lore2"))document.getElementById("lore2").innerHTML = "You won but since lore does not make sense you brought a totem of revival with yourself so you did not die at all 💀💀💀"
  }
  if(document.getElementById("decelerate")){
    document.getElementById("decelerate").innerHTML = (player.decelerated ? "Die Again" : "Die")
  }
...
const UPGRADES = {
  1: {
    title: "Die, you foolish worm!",
    desc: "Generate 300 energy per second.",
    cost: new Decimal(10),
    effectDisplay() {return null},
    pair: 0,
  },
  2: {
    title: "Revive, you smart human!",
    ...
    effect() {
      switch (player.timesEscaped+1){
        ...
        case 4:
        case 9:
          return new Decimal(player.buttonPresses[1]).gte(1) ? new Decimal(player.buttonPresses[1]).pow(1.2).add(1) : new Decimal(player.buttonPresses[1]).pow(1.2).add(1)
        break;
      }
    },
  ...
  3: {
    ...
    effect() {
      switch (player.timesEscaped+1){
        ...
        case 7:
        case 8:
          return new Decimal(player.buttonPresses[1]).gte(50) ? new Decimal(player.buttonPresses[1]).pow(1.2).add(1) : new Decimal(player.buttonPresses[1]).pow(1.2).add(1)
        break;
        case 4:
        case 9:
          return new Decimal(player.buttonPresses[1]).pow(0.75).add(1)
  ...
  4: {
    ...
    effect() {
      switch (player.timesEscaped+1){
        ...
        case 8:
        case 9:
          return new Decimal(player.buttonPresses[4]).pow(2.4).add(1).min(500)
        break;
        case 2:
        case 5:
        case 6:
        case 7:
          return new Decimal(player.buttonPresses[4]).pow(2.4).add(1)
        break;
      }
    },
  ...
  5: {
    title: "Nahh, die again and never revive again",
    desc: "Gain more energy based on Blue Button presses. (You won't be able to buy the other upgrade)",
    cost: new Decimal(5000),
    effect() {
      switch (player.timesEscaped+1){
  ...
  6: {
    title: "Among Us",
    desc: "Gain more energy based on All Button presses. (You won't be able to buy the other upgrade)",
    cost: new Decimal(1e10),
    effect() {
      return new Decimal(player.buttonPresses[1]).add(player.buttonPresses[2]).add(player.buttonPresses[3]).add(player.buttonPresses[4]).pow(2.5).add(1)
    },
...
const BUYABLES = {
  ...
  2: {
    desc: "4x button presses per purchase",
    cost() {
      return new Decimal(2).mul(Decimal.pow(3,player.buyables[2]))
    },
    effect() {
      return player.challenge == 2 ? 1 : new Decimal(4).pow(player.buyables[2]).min(1e300).toNumber()
    },
    effectDisplay() {
  ...
  3: {
    desc: "NaN CP gain per purchase",
    cost() {
      return new Decimal(10).mul(Decimal.pow(4,player.buyables[3]))
    },
    effect() {
      return player.challenge == 2 ? new Decimal(1) : Decimal.pow(hasEpsUpgrade(6)?2.2:6.9420,player.buyables[3])
    },
    effectDisplay() {
...
const DECEL_BUYABLES = {
  1: {
    desc: "Multiply DP gain by x3.1415 per purchase",
    cost() {
      return new Decimal(10).mul(Decimal.pow(5,player.buyables[5]))
    },
    effect() {
      return player.challenge == 2 ? new Decimal(1) : Decimal.pow(3.1415,player.buyables[5])
    },
    effectDisplay() {
  ...
  2: {
    desc: "Make the time speed formula a lot better",
    cost() {
  ...
  3: {
    desc: "Multiply energy gain by 69 per purchase",
    cost() {
      return new Decimal(1000).mul(Decimal.pow(10,player.buyables[7]))
    },
    effect() {
      return player.challenge == 2 ? new Decimal(1) : Decimal.pow(new Decimal(69).mul(CONDENSERS[2].effect2()),player.buyables[7])
    },
    effectDisplay() {
...
const SUPER_BUYABLES = {
  1: {
    desc: "Triple generator production per purchase",
    cost() {
      return new Decimal(1e9).mul(Decimal.pow(10**(1+player.buyables[8]/3),player.buyables[8]))
    },
    effect() {
      return hasEscUpgrade(16) ? Decimal.pow(3,player.buyables[8]+player.buyables[9]) : Decimal.pow(2,player.buyables[8])
    },
    effectDisplay() {
  ...
  2: {
    desc: "Add 2 to the Super-Energy effect exponent per purchase",
    cost() {
...
const ESC_UPGRADES = {
  1: {
    title: "Clicking Bozo",
    desc: "Gain 25x more energy from clicking and generating.",
    cost: new Decimal(1),
    effectDisplay() {null},
  },
  2: {
  ...
  11: {
    title: "Synergism (Tax Land)",
    desc: "Unlock Auto-Escape and CP and DP boost each other.",
    cost: new Decimal(6e7),
    effect() {return player.deceleratePower.max(1).log(2.324).add(1)},
    effect2() {return player.countdownPoints.max(1).log(1.4894729848920348238952749052390572489230840293524859024859023784923738924748923752389748932569034804237423894237234947298).add(1)},
    effectDisplay() {return format(ESC_UPGRADES[11].effect2()) + "x CP, " + format(ESC_UPGRADES[11].effect()) + "x DP"},
  },
  12: {
    title: "Cringe Time II",
    desc: "The Time Speed formula is significantly better.",
    cost: new Decimal(2e8),
    effectDisplay() {return null},
  },
  13: {
    title: "ran out of names,",
